...
function NumInput({
  ...
  return (
    <input
      ...
      className="w-16 bg-gray-900 text-gray-200 text-xs font-mono rounded px-1 py-0.5 border border-gray-700 focus:outline-none focus:border-blue-500"
    />
  )
}
  ...
  return (
    <div
      className={`rounded-lg p-2 mb-2 border ${
        part.type === 'movable' ? 'border-blue-800 bg-blue-950/20' : 'border-gray-700 bg-gray-800/40'
      }`}
    >
      <div className="flex justify-between items-center mb-1.5">
        <span className="text-xs font-semibold text-gray-200">{part.name}</span>
        <span
          className={`text-xs px-1.5 py-0.5 rounded ${
            part.type === 'movable' ? 'bg-blue-700 text-blue-100' : 'bg-gray-700 text-gray-300'
          }`}
        >
      ...
      <div className="grid grid-cols-3 gap-x-2 gap-y-1 text-xs text-gray-400">
        <span>W:</span>
